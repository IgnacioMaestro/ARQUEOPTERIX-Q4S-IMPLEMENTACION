...
bool Q4SServerProtocol::handshake(Q4SSDPParams &params)
{
    ...
    if( ok )
    {
		...
		ok &= mServerSocket.sendTcpData( DEFAULT_CONN_ID, Q4SMessage_create200OKBeginResponse(params).c_str());
    }
...
void Q4SServerProtocol::continuity(Q4SSDPParams params)
{
    ...
    while ( !stop )
    {
        ...
        {
            //Alert
            std::string alertMessage = Calculation_generateNotificationAlertMessage(params, upResults, results);
...
bool Q4SServerProtocol::ready()
{
    ...
    if( ok )
    {
		ok &= mServerSocket.sendTcpData( DEFAULT_CONN_ID, Q4SMessage_create200OKBeginResponse().c_str());
    }
...
bool Q4SServerProtocol::measureStage0(Q4SSDPParams params, Q4SMeasurementResult &results, Q4SMeasurementResult &upResults, unsigned long pingsToSend)
{
    ...
    if (ok)
    {
		Sleep(q4SServerConfigFile.timeStartCalc);

		// Calculate Latency
		Calculation_calculateLatency(
			mReceivedMessages, 
        ...
        // Calculate Jitter
        Calculation_calculateJitterStage0(
			mReceivedMessages, 
    ...
    if ( ok )
    {
		...
		ok &= Calculation_checkStage0(params.latency, params.jitterUp, params.latency, params.jitterDown, upResults, results);
	}

	if (!ok)
	{
		Calculation_showCheckMessage(upResults, results);
	}
...
bool Q4SServerProtocol::interchangeMeasurementProcedure(Q4SMeasurementValues &upMeasurements, Q4SMeasurementResult results)
{
    ...
    if ( ok )
    {
		...
		ok &= mServerSocket.sendTcpData(DEFAULT_CONN_ID, Q4SMessage_createPing("myIp", q4SServerConfigFile.defaultUDPPort, 0, 0, results.values).c_str());
    }
...
bool Q4SServerProtocol::measureContinuity(Q4SSDPParams params, Q4SMeasurementResult &results, Q4SMeasurementResult &upResults, unsigned long pingsToSend)
{
    ...
    if (ok)
    {
        ...
        // Calculate Jitter
        Calculation_calculateJitterAndPacketLossContinuity(
			mReceivedMessages, 
			results.values.jitter,
	...
	if (ok)
	{
		...
		ok &= Calculation_checkContinuity(
...
bool Q4SServerProtocol::sendRegularPings(std::vector<unsigned long> &arrSentPingTimestamps, unsigned long pingsToSend, unsigned long timeBetweenPings)
{
    ...
    for( pingNumber = 0; pingNumber < pingNumberToSend; pingNumber++ )
    {
		...
		ok &= mServerSocket.sendUdpData( DEFAULT_CONN_ID, Q4SMessage_createPing("myIp", q4SServerConfigFile.defaultUDPPort, pingNumber, timeStamp).c_str());
...
bool Q4SServerProtocol::measureStage1(Q4SSDPParams params, Q4SMeasurementResult &results, Q4SMeasurementResult upResults)
{
	...
	{
		std::string message = Q4SMessage_createBWidth("myIp", q4SServerConfigFile.defaultUDPPort, sequenceNumber);
		ok &= mServerSocket.sendUdpData(DEFAULT_CONN_ID, message.c_str());
	...
	if (ok)
    {
		Calculation_calculateBandwidthStage1(sequenceNumber, params.procedure.bandwidthTime, results.values.bandwidth);
	...
	if (ok)
	{
        ...
        bool okCalculated = Calculation_calculatePacketLossStage1(mReceivedMessages, results.values.packetLoss);
		if (!okCalculated)
		{
	...
	if (ok)
	{
		...
		ok &= Calculation_checkStage1(params.bandWidthUp, params.packetLossUp, params.bandWidthDown, params.packetLossDown, upResults, results);
	}
    ...
    while ( ok )
    {
        ...
        if( ok )
        {
            ...
            if ( Q4SMessageTools_isPingMessage(udpBuffer, &pingNumber, &receivedTimeStamp) )
            {
				...
				ok &= mServerSocket.sendUdpData( connId, Q4SMessage_create200OKBeginResponse(pingNumber).c_str() );
